...
output:
   html_notebook:
    ...
    toc_depth: 2
---
## reviewer : package lattice has also to be installed 
...
## Data Preparation
## reviewer: str functions shows us that this dataset contains 13 variables 
## reviewer : first variable do not contain any relvant information so it's good that it has been removed.
## reviewer : NA's has been also removed 
```{r}
url <- "KNN-hcvdat0.csv"
...
## counting 
## reviewer: we can look at the counts and proportions for both outcomes using the tables and the prop.tables functions
```{r}
...
## Change Type
## reviewer : changing the variable from character to factor
```{r}
...
## reviewer : building a function to rescale some unscaled variabels
```{r}
...
# reviewer : deleting unnecessary columns
```{r}
NorDF <- cleanDF2[4:13]
```
...
## reviewer: the normalize functions has been applied to each feature in the dataset.
```{r}        
...
## reviewer: creating training and testing sets. but without the labels. 
```{r}
...
## Making Test labels
## reviewer : creating a seperate training and testing sets for labels 
```{r}
